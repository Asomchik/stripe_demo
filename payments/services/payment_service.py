...
class PaymentServices:
    ...
    def _apply_discount(cls, order):
        if order.discount:
            stripe.api_key = STRIPE_SECRET_KEY
            coupon = stripe.Coupon.create(percent_off=order.discount.value, duration='once')
            return coupon.id
    ...
    def _apply_tax(cls, order):
        if order.tax:
            stripe.api_key = STRIPE_SECRET_KEY
            tax = stripe.TaxRate.create(display_name=order.tax.name, inclusive=False, percentage=order.tax.value)
            return tax.id
